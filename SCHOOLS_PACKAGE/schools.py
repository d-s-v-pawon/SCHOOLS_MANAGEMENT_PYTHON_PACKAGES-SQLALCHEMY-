...
import random
import copy
from tabulate import tabulate                   #The Following modules should be imported before creating the class SCHOOLS
import string
from STUDENTS_PACKAGE import students
import json
from sqlalchemy import create_engine , Column , Integer ,String , BIGINT ,Float
from sqlalchemy.orm import declarative_base

engine = create_engine('mssql+pyodbc://@LAPTOP-04Q912VM\SQLEXPRESS/SCHOOL_EDUCATION?driver=ODBC+Driver+17+for+SQL+Server&Trusted_connection=yes')
...
class SCHOOLS:              #creating a class named SCHOOLS
    ...
    def C_YEAR_DET(self):                                   #The school object describes the current students details of the specified school using this method
        ...
        class thisyear(Base):
            __tablename__ = f"C_YEAR_{self.sch_name}_SCHOOL"

            NAMES = Column(String(30),nullable=False)
            GENDERS = Column(String(10),nullable=False)
            AGES = Column(Integer,nullable=False)
            SCHOOL_NAMES = Column(String(30),nullable=False)
            STANDARDS =Column(Integer,nullable=False)
            SECTIONS = Column(String(2),nullable=False)
            ROLL_NUMBERS = Column(Integer,nullable=False)
            CONTACT_NUMBERS = Column(BIGINT,primary_key=True,nullable=False)
        ...
        for i in self.CY_SCHOOL:
            for j in i[1]:
                stu = thisyear(NAMES = j.name , GENDERS = j.gender , AGES = j.age , SCHOOL_NAMES = j.sch_name , STANDARDS = j.standard , SECTIONS = j.section , ROLL_NUMBERS = j.r_no , CONTACT_NUMBERS = j.c_no , PERCENTAGES = j.percent)
                session.add(stu)
    ...
    def N_YEAR_DET(self):                       #The school object describes the students details of the school specified using this method
        ...
        class nextyear(Base):
            __tablename__ = f"N_YEAR_{self.sch_name}_SCHOOL"
        ...
        self.STU_TABLE = nextyear.__table__
        ...
        for i in self.NY_SCHOOL:
            for j in i[1]:
                stu = nextyear(NAMES = j.name , GENDERS = j.gender , AGES = j.age , SCHOOL_NAMES = j.sch_name , STANDARDS = j.standard , SECTIONS = j.section , ROLL_NUMBERS = j.r_no , CONTACT_NUMBERS = j.c_no , PERCENTAGES = j.percent)
                session.add(stu)
    ...
    def NEW_ADMISSIONS(self,name,gender,age,standard,c_no,percent):                 #The school object specifies the new admission details of the students into the school using this method
        ...
        stu = self.STU_TABLE(NAMES = n_stu.name , GENDERS = n_stu.gender , AGES = n_stu.age , SCHOOL_NAMES = n_stu.sch_name , STANDARDS = n_stu.standard , SECTIONS = n_stu.section , ROLL_NUMBERS = n_stu.r_no , CONTACT_NUMBERS = n_stu.c_no , PERCENTAGES = n_stu.percent)
